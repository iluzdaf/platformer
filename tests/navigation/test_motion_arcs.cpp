...
TEST_CASE("An actor with nothing to fall with has no fall arc", "[MotionArcs]")
{
    ActorMotionData motionData;
    motionData.moveAbilityData = MoveAbilityData{};

    REQUIRE(simulateFallArc(motionData).empty());
}

TEST_CASE("A fall arc ends below where it left", "[MotionArcs]")
{
    std::vector<glm::vec2> arc = simulateFallArc(jumperMotionData());
...
TEST_CASE("A fall never rises", "[MotionArcs]")
{
    std::vector<glm::vec2> arc = simulateFallArc(jumperMotionData());

    for (size_t index = 1; index < arc.size(); ++index)
        REQUIRE(arc[index].y >= arc[index - 1].y);
}

TEST_CASE("Walking off faster lands further out", "[MotionArcs]")
{
    REQUIRE(reachOf(simulateFallArc(jumperMotionData(), 0.5f)) <
            reachOf(simulateFallArc(jumperMotionData(), 1.0f)));
}

TEST_CASE("Stepping off drops straight down", "[MotionArcs]")
{
    REQUIRE(reachOf(simulateFallArc(jumperMotionData(), 0.0f)) == 0.0f);
}

TEST_CASE("An actor is offered a range of ways off a ledge", "[MotionArcs]")
{
    std::vector<std::vector<glm::vec2>> arcs = simulateFallArcs(jumperMotionData());

    REQUIRE(arcs.size() > 1);
    for (size_t index = 1; index < arcs.size(); ++index)
        REQUIRE(reachOf(arcs[index]) < reachOf(arcs[index - 1]));
}
