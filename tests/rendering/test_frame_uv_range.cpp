...
TEST_CASE("Frame 0 is the top left cell of the sheet", "[FrameUvRange]")
{
    auto [start, end] = frameUvRangeIn(96, 96, 0, 32, 32);

    REQUIRE(start.x == Approx(0.0f));
    REQUIRE(end.x == Approx(1.0f / 3.0f));
    REQUIRE(start.y == Approx(2.0f / 3.0f));
    REQUIRE(end.y == Approx(1.0f));
}

TEST_CASE("The frame after a row's last is on the row below it", "[FrameUvRange]")
{
    auto [first, firstEnd] = frameUvRangeIn(96, 96, 0, 32, 32);
    auto [next, nextEnd] = frameUvRangeIn(96, 96, 3, 32, 32);

    REQUIRE(next.x == Approx(first.x));
    REQUIRE(next.y == Approx(first.y - 1.0f / 3.0f));
    REQUIRE(nextEnd.y == Approx(firstEnd.y - 1.0f / 3.0f));
}
...
TEST_CASE("Each axis of a sheet is divided by its own size", "[FrameUvRange]")
{
    ...
    REQUIRE(start.x == Approx(0.0f));
    REQUIRE(start.y == Approx(128.0f / 192.0f));
    REQUIRE(end.y == Approx(160.0f / 192.0f));
}
...
TEST_CASE("A sheet shorter than one row of cells reads as the whole sheet", "[FrameUvRange]")
{
    auto [start, end] = frameUvRangeIn(96, 16, 0, 32, 32);
